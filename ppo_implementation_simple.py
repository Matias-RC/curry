...
def run_episode(env:Environment, agent):
    ...
    x_ = F.one_hot(
    torch.tensor(env.state[1], dtype=torch.long),
    num_classes=3
    ).float()
    x = torch.cat((x,x_), 0)
    while not env.finished:
        with torch.no_grad():
            ...
            step_logs.append((x, action.item(), xp, reward, env.finished, logp_old))
            total_steps += 1
...
def offline_train(batch, agent, optimizer):
    ...
    A = torch.tensor([a_t for s_t, a_t, s_tplus1, r_t, done, logp_old in batch], device=S.device)
