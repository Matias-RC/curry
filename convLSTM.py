...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor, hidden_state=None):
        ...
        if hidden_state is None:
            ...
            if not (isinstance(hidden_state, (tuple, list)) and len(hidden_state) == 2):
                raise ValueError("hidden_state must be a tuple/list (h, c)")
            h_prev, c_prev = hidden_state
            if h_prev.shape[0] != b or c_prev.shape[0] != b:
                raise ValueError("Batch size mismatch between input and hidden_state")
            if h_prev.shape[1] != self.hidden_dim or c_prev.shape[1] != self.hidden_dim:
                raise ValueError(f"hidden_state channels must be {self.hidden_dim}")
            if (h_prev.shape[2], h_prev.shape[3]) != (h, w) or (c_prev.shape[2], c_prev.shape[3]) != (h, w):
                raise ValueError("hidden_state spatial dims must match input spatial dims")
            if h_prev.device != input_tensor.device or c_prev.device != input_tensor.device:
                raise ValueError("hidden_state device must match input_tensor device")
...
class StackedConvLSTM(nn.Module):
    ...
    def forward(self,
        ...
        # check that input matches expected layer 0 input_dim
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if hidden_state is None:
                h_prev, c_prev = layer._init_hidden(batch_size=b, image_size=(h, w))
            else:
